...
### **【V4.2 章节扩容】完整事件闭环原则 (Complete Event Closure)**

**【核心理念转变】**
一个章节不应该仅仅是"一个场景片段"，而应该是"一个完整的事件闭环"。

**传统章节（错误示范）:**
- 节拍1-4：角色从客厅走到房间，章节结束
- **问题:** 事件没有闭环，玩家感觉"被截断"

**V4.2完整章节（正确示范）:**
- 节拍1-3：回到家，重逢场景
- 节拍4-6：被安置到房间，换衣服，初步安顿
- 节拍7-9：独处时的回忆与情感消化
- 节拍10-12：晚餐或简短交谈，建立新常态
...
- **Internal Transition** (内部转场): 时间跳跃或场景切换（NEW）
- **Reflection** (反思): 角色独处时的内心戏（NEW）

**【终章信标重新定义】**
终章信标不应该是"离开房间"这样的小动作，而应该是"**这一整段剧情彻底结束**"的标志：

**错误示例:**
\`\`\`json
"endgame_beacons": ["当Yumi离开客厅，走向楼梯时。"]
\`\`\`
→ **问题:** 仅仅是场景切换，事件未闭环
...
\`\`\`json
"endgame_beacons": [
  "当Yumi在新房间中安顿下来，窗外的月光照进来，她终于独自一人面对这个陌生又熟悉的环境，第一天的疲惫让她开始准备休息时。"
]
\`\`\`
→ **正确:** 描述了"回家第一天"这个完整事件的结束
...
**节拍结构（共10个）:**
1. **Action** - 掉落的书籍与弹起的红犬（Rofi从沙发弹起）
2. **Action** - 激动的碰撞（Rofi冲向Yumi，差点撞倒）
3. **Dialogue Scene** - 破碎的问候（语无伦次的重逢对话）
4. **Action** - Theo的介入（被打断，要求去换衣服）
5. **Transition** - 上楼与引导（被带到客房）
6. **Action** - 换衣与清洁（处理湿透的衣物）
7. **Internal Transition** - "一个小时后，楼下传来晚餐的呼唤..."
8. **Dialogue Scene** - 餐桌上的试探性交谈（三人共进晚餐）
9. **Reflection** - 独处的回忆（回到房间后的内心独白）
10. **Action** - 准备休息（整理床铺，看向窗外雪景）

**终章信标:**
"当Yumi熄灯躺下，窗外的暴风雪声音渐渐变成白噪音，她闭上眼睛，今天的所有情绪——寒冷、重逢、陌生、温暖——在黑暗中慢慢沉淀，第一夜就这样开始了。"

**闭环完成度:**
✅ 完整的"第一天回家"事件：从进门→安顿→晚餐→休息
✅ 情感弧线完整：激动→被打断→独处→平静
✅ 时间跨度合理：从傍晚到夜晚
✅ 玩家不会感觉"被截断"
...
\`\`\`json
{
  ...
  "chapter_blueprint": {
    ...
    "plot_beats": [
      {
        "beat_id": "【节拍1】: ...",
        "type": "[节拍类型：'Action' (动作) / 'Dialogue Scene' (对话场景) / 'Transition' (外部转场) / 'Internal Transition' (内部转场) / 'Reflection' (反思)]",
        "description": "[对节拍的简洁描述]",
        "exit_condition": "[仅当 type 为 'Dialogue Scene' 时提供。描述一个概念性的、可被对话内容触发的场景结束条件。]"
      },
      {
        "beat_id": "【节拍2】: ...",
        "type": "...",
